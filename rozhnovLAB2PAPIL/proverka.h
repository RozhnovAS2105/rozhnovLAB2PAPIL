...
//.fail проверка на тип данных
template <typename T> //вывод существующих айдишников 
ostream& operator<< (ostream& out, unordered_map <int, T>& p) {
    out << "Exiting id: ";
    for (auto& [i, obj] : p)//проходимся по каждому элементу в словаре 
    {
        out << i << " ";
    }
    out << endl;
...
template <typename T>//шаблон для функции и указателей
